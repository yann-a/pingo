...
func eval(e expr, envir *env, wg *sync.WaitGroup) {
	...
	case parallel:
		for _, task := range v {
			wg.Add(1)
			// We do every task in parallel in separate goroutines
	...
	case receiveThen:
		channel := envir.get_value(variable(v.channel), wg).(channel)
		privateChan := make(chan value) // We open a private channel to communicate with the proxy

		channel.request <- privateChan
		message := <-privateChan // We wait for its answer on that channel
